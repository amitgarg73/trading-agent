"""Tests for agents/news_intel.py — Strategy A earnings blackout is ACTIVE."""
...
# ---------------------------------------------------------------------------
# run() — earnings blackout IS active in Strategy A
# ---------------------------------------------------------------------------
...
def test_run_mixed_candidates_only_earnings_blocked():
    today = date.today()
...
def test_run_news_context_built_for_passing_candidates():
    with patch("agents.news_intel._get_earnings_date", return_value=None), \
