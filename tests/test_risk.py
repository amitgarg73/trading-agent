...
class TestValidateTrade:
    ...
    def test_rr_below_minimum_rejected(self):
        ...
        ok, msg = _validate_trade(make_trade(entry=100, target=100.5, stop=99.5, size=5000))
        assert not ok
...
class TestRiskRun:
    ...
    def test_shares_populated_on_approval(self):
        out = run(self._make_strategy_output([make_trade(entry=100, size=6000)]))
        trade = out["approved_trades"][0]
        assert trade["shares"] == 60
        assert trade["status"] == "PLANNED"

    def test_estimated_profit_correct(self):
        # entry=100, target=102, shares=60 → profit = 60*(102-100) = $120
        out = run(self._make_strategy_output([make_trade(entry=100, target=102, size=6000)]))
        trade = out["approved_trades"][0]
        assert trade["estimated_profit"] == pytest.approx(120.0, abs=1.0)

    def test_max_loss_correct(self):
        # entry=100, stop=99.50, shares=60 → loss = 60*(100-99.50) = $30.00
        out = run(self._make_strategy_output([make_trade(entry=100, stop=99.50, size=6000)]))
        trade = out["approved_trades"][0]
        assert trade["max_loss"] == pytest.approx(30.0, abs=1.0)
