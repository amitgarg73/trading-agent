...
TRAIL_PCT                   = 0.025       # Trailing stop: close if price drops 2.5% from highest seen since entry (widened from 1.5% — 1.5% fires on normal chop for 2-4% ATR stocks before capturing the move)
...
INTRADAY_ENTRY_CUTOFF_UTC       = 16          # 12:00 PM ET hard entry cutoff; data shows 0 targets + 12-25% win rate after noon
INTRADAY_SCAN_MAX_RUNS          = 2           # only 10:30 AM + 11:30 AM scans now run; cap reduced accordingly
INTRADAY_SCAN_MIN_INTERVAL_MINS = 50          # 50 min minimum — entry_scan runs hourly; prevents double-fire
...
PREMARKET_MIN_SCORE  = 4          # premarket pre-filter: lowered 5→4 2026-06-01 (score≥5 produces 0 candidates in current market)
