...
def _fetch_orb_pct(ticker: str, entry: float) -> float | None:
    """Opening range (9:30–9:59 AM ET) as fraction of price via Alpaca 1-min bars. Cached per process."""
...
def apply(trades: list[dict],
    ...
    for t in trades:
        ...
        # Size floor and R:R check BEFORE ORB halving — ORB is intentional risk reduction, not a quality gate.
